...
- **ABAC (Attribute-Based Access Control)** - 基于属性的访问控制（2010）：动态属性判断（部门+职级+时间+地理位置）
...
- 在**单体架构**中，鉴权散落在 Controller 注解中（@PreAuthorize），这是**防御式编程**——每个开发者都是安全工程师，必然出现漏洞
- 在**云原生架构**中，鉴权被横切（Cross-cutting）到 Gateway 和 Sidecar，这是**声明式安全**——安全策略通过 YAML 配置，与业务代码生命周期解耦
...
- **数据层（业务逻辑）** 细粒度授权（能否查看"部门A的数据"）：属于业务层，应在 Service 层处理

**关键洞察**：RBAC 只解决"角色分配"，真正的业务规则（如"部门经理只能看本部门数据"）必须下沉到领域模型。
...
| 演进阶段 | 认证 | 授权 | 鉴权 |
|----------|------|------|------|
| 单体时代 | Session/Cookie | ACL/RBAC | 代码注解 |
| 微服务时代 | JWT/OAuth2 | ABAC/PBAC | API Gateway + Sidecar |
| 零信任时代 | 持续验证/Passkey | 策略即代码 | 全链路身份追踪 |

**最终结论**：认证、授权、鉴权的三分法，本质上是**关注点分离原则在安全领域的具象化**。随着系统规模增长，这三者会经历"混沌→耦合→分离"的完整生命周期。理解这一规律，才能在架构演进中做出正确决策。

---

**来源**：[认证、授权与鉴权：架构演进中的边界划分艺术 - 掘金](https://juejin.cn/post/7615431891068354612)
