...
**重要**: 遵循 TenantService 和 DepartmentService 的实现模式。
...
7. **Service 接口**
   - 路径: `domain/service/<实体名称>Service.java`
   - **关键**: 继承 `ClosureService<Entity, EntityProxy, 闭包实体, 闭包实体Proxy>`

8. **Service 实现类**
   - 路径: `application/service/<实体名称>ServiceImpl.java`
   - **关键**: 继承 `ClosureEntityService<Entity, EntityProxy, 闭包实体, 闭包实体Proxy>`
   - 实现抽象方法:
     ```java
     @Override
     protected SQLActionExpression2<IncludeContext, EntityProxy> navigate() {
         return (c, t) -> c.query(t.ancestors());
     }

     @Override
     protected SQLFuncExpression1<EntityProxy, SQLSelectExpression> setColumnParentId() {
         return EntityProxy::parentId;
     }

     @Override
     protected 闭包实体 createClosure(UUID ancestorId, UUID descendantId, Integer distance) {
